...
interface User {
  id: number;
  user_type: string; // ユーザータイプを文字列で管理
}
...
const Navbar = () => {
  ...
  const [userType, setUserType] = useState<string | null>(null); // ユーザータイプを追加
    ...
    if (userId) {
      axios
        ...
        .then((response) => {
          ...
          setUserType(response.data.user_type); // ユーザータイプを設定
        })
        .catch((error) => {
    ...
    <nav className="bg-gradient-to-r from-blue-500 to-indigo-600 shadow-md">
      <div className="container mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center space-x-6">
          ...
          {userType === 'company' && ( // ユーザータイプが企業の場合のみ表示
